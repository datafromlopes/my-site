(() => {
  ...
  (function() {
    ...
    function closeCollapse() {
      ...
      if (!isOpen) return;
      try {
        if (window.bootstrap?.Collapse) bootstrap.Collapse.getOrCreateInstance(collapseEl, { toggle: false }).hide();
    ...
    document.addEventListener("click", function(ev) {
      const a = ev.target.closest("#navbarResponsive a.nav-link, #navbarResponsive a.dropdown-item, #sideNav a.navbar-brand");
      if (!a) return;
