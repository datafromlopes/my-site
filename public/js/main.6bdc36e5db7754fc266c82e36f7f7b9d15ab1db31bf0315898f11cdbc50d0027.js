(() => {
  ...
  (function() {
    ...
    function renderPage() {
      ...
      if (resultsInfo) {
        const showing = Math.min(end, filteredProjects.length);
        if (filteredProjects.length === 0) {
          resultsInfo.textContent = "No projects found";
        } else {
          resultsInfo.textContent = `Showing ${start + 1}-${showing} of ${filteredProjects.length} projects`;
        }
      }
    ...
    if (tagButtons.length) {
      tagButtons.forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          filterProjects(currentTag);
        });
      });
    }
    filterProjects("__all");
  })();
})();
